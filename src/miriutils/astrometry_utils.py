...
Example Usage:
--------------
    import miriutils.astrometry_utils as au

    # Specify directories for the reference catalogue
    catalogue = './catalogues/target_coordinates.fits'
    
    # Define a look-up pattern for the cutout files to be followed by the code
    # Based on the directory tree generated by the CutoutManager
    MIRI_TEMPLATE = "./cutouts/{survey_name}/fits/{id}_{filter}_{survey_name}{obs}.fits"
    
    # Specify NIRCam file pattern to compare the cutouts with MIRI
    NIRCAM_TEMPLATE = "./NIRCam/F444W_cutouts/{id}_F444W_cutout.fits"

    # Specify the output directory
    base_out = "/Users/benjamincollins/University/Master/Red_Cardinal/astrometry/"
    
    # Define which filters belong to which survey
    survey_config = {
        "primer1": ["F770W", "F1800W"],
        "primer2": ["F770W", "F1800W"],
        "cos3d1":  ["F1000W", "F2100W"],
        "cos3d2":  ["F1000W", "F2100W"]
    }
    ...
    for survey, filters in survey_config.items():
        for filt in filters:
            au.compute_offset(
                ...
                miri_template=MIRI_TEMPLATE,
                nircam_template=NIRCAM_TEMPLATE
            )
